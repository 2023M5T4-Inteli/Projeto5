...
function solicitarIndenizacao( address _endereco, bytes15 _bo) public {
    Participante storage participante = meuGrupo.mapeamentoParticipantes[_endereco]; //instancia o participante dentro do grupo
    require(participante.saldo >=   participante.valorAtivo * 5/100, "Saldo insuficiente para solicitar indenizacao");//Verifica se o cliente tem saldo suficiente

    emit pedidoIndenizacao(msg.sender, _bo);//envia notificação para segurado com o endereço e bo do participante
    participante.pediuIndenizacao = true;
    
}
//Função de indenizar o cliente
